...
const columnHelper = createColumnHelper<TypeSupport>()
...
const TableMarketing = ({ items }: DataTable) => {
  ...
  const handleClickOpenEdit = (data: TypeSupport) => () => {
    setSelect(data)
    setOpenEdit(true)
  };

  const handleClickOpenDel = (data: TypeSupport) => () => {
    setSelect(data)
    setOpenDel(true)
  };
  ...
  const columns = useMemo<ColumnDef<TypeSupport, any>[]>(
    () => [

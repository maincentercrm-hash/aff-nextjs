...
const columnHelper = createColumnHelper<TypeSetting>()
...
const TableSetting = ({ items }: DataTable) => {

  const { open, setSelect, setOpenDel, setOpenEdit, handleStatusChange } = useContext(ItemsContext);
  ...
  const handleClickOpenEdit = (data: TypeSetting) => () => {
    setSelect(data)
    setOpenEdit(true)
  };

  const handleClickOpenDel = (data: TypeSetting) => () => {
    setSelect(data)
    setOpenDel(true)
  };
  ...
  const columns = useMemo<ColumnDef<TypeSetting, any>[]>(
    () => [
      ...
      columnHelper.accessor('point', {
        cell: info => info.getValue(),
        header: 'POINT ที่จะได้รับ'
      }),


      // เพิ่มคอลัมน์วันที่สร้าง

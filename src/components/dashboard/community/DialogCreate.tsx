...
import { communitySchema } from '@/validations/schemas'
...
const DialogCreate = ({ initDefault, structure }: propsCreate) => {
  ...
  // Validation
  const { validate, getError, clearErrors, hasErrors } = useFormValidation(communitySchema)

  // States
  const [open, setOpen] = useState<boolean>(false)
  const [data, setData] = useState(initDefault)
  const [status, setStatus] = useState<string>(initDefault.status)
  const [category, setCategory] = useState<string>(initDefault.category)
  ...
  const handleCategory = (e: SelectChangeEvent<typeof status> | any) => {

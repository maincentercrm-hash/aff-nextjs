...
const columnHelper = createColumnHelper<TypeMarketing>()
...
const TableMarketing = ({ items }: DataTable) => {
  ...
  const handleClickOpenEdit = (data: TypeMarketing) => () => {
    setSelect(data)
    setOpenEdit(true)
  };

  const handleClickOpenDel = (data: TypeMarketing) => () => {
    setSelect(data)
    setOpenDel(true)
  };
  ...
  const columns = useMemo<ColumnDef<TypeMarketing, any>[]>(
    () => [

      {
        ...
        accessor: 'thumbnail',
        cell: info => (
          <img src={info.row.original.thumbnail} className='w-full max-h-[100px] object-contain' alt={info.row.original.title} />
        ),
        header: 'ภาพปก'
      },

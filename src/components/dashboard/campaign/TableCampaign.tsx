...
import type { TypeCampaign } from '@/types/typeCampaign'
...
const columnHelper = createColumnHelper<TypeCampaign>()
...
const TableCampaign = ({ items }: DataTable) => {

  const { open, setSelect, setOpenDel, setOpenState } = useContext(ItemsContext);

  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
  const [globalFilter, setGlobalFilter] = useState('')
  const [data, setData] = useState(items);

  const handleClickOpenState = (data: TypeCampaign) => () => {
    setSelect(data)
    setOpenState(true)
  };

  const handleClickOpenDel = (data: TypeCampaign) => () => {
    setSelect(data)
    setOpenDel(true)
  };
  ...
  const columns = useMemo<ColumnDef<TypeCampaign, any>[]>(
    () => [

      {
        ...
        accessor: 'thumbnail',
        cell: info => (
          <Image src={info.row.original.thumbnail} width={100} height={100} alt={info.row.original.title} />
        ),
        header: 'ภาพปก'
      },
...

      {
        ...
        cell: (info) => (
          <div className="flex gap-0">
            <Chip
              label={`${((info.row.original.active / info.row.original.volumn) * 100).toFixed(2)}%`} // Calculate percentage and format to 2 decimal places
              color="success"
              variant="outlined"
            />
          </div>
...

      {
        ...
        cell: info => (
          <div className='flex gap-0'>
            ...
            <IconButton aria-label='capture screenshot' size='large' onClick={handleClickOpenState(info.row.original)}>
              <i className='tabler-report-analytics text-[28px]' />
            </IconButton>

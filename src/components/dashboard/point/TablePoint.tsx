...
const columnHelper = createColumnHelper<TypePoint>()
...
const TableMarketing = ({ items }: DataTable) => {
  ...
  const handleClickOpenEdit = (data: TypePoint) => () => {
    setSelect(data)
    setOpenEdit(true)
  };

  const handleClickOpenDel = (data: TypePoint) => () => {
    setSelect(data)
    setOpenDel(true)
  };
  ...
  const columns = useMemo<ColumnDef<TypePoint, any>[]>(
    () => [
      ...
      columnHelper.accessor('type', {
        cell: info => {
          ...
          return (
            <span className={`px-2 py-1 rounded text-xs font-medium ${type === 'credit' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
              {type || 'default'}
            </span>
          )
        },
